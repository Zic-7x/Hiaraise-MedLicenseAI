...
export default function AdminSlotManager() {
  ...
  const [editForm, setEditForm] = useState({ date: '', start_time: '', end_time: '', is_available: true });
  ...
  useEffect(() => {
    const subscription = supabase
      ...
      .on('postgres_changes', 
        ...
        (payload) => {
          // When a new booking is made, close the corresponding slot
  ...
  const handleAdd = async (e) => {
    ...
    const { error } = await supabase.from('call_slots').insert([
  ...
  const handleUpdate = async (id) => {
    ...
    const { error } = await supabase.from('call_slots').update(editForm).eq('id', id);
  ...
  const handleDelete = async (id) => {
    setLoading(true);
    setError('');
    const { error } = await supabase.from('call_slots').delete().eq('id', id);
  ...
  return (
    <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl shadow-xl p-8 max-w-4xl mx-auto mt-8">
      <h2 className="text-2xl font-bold bg-gradient-to-r from-white to-cyan-400 bg-clip-text text-transparent mb-6 text-center">Manage Call Slots</h2>
      
      {/* Auto-close info */}
      <div className="bg-blue-900/20 border border-blue-400/30 rounded-lg p-4 mb-6">
        <h3 className="text-blue-400 font-semibold mb-2">🔄 Automatic Slot Management</h3>
      ...
      <form onSubmit={handleAdd} className="flex flex-wrap gap-3 items-end mb-8">
        <input
          ...
          className="p-2 rounded-lg bg-white/20 border border-white/30 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-cyan-400"
          required
        />
        <input
          type="time"
        ...
        <button
          type="submit"
          className="bg-gradient-to-r from-cyan-500 to-blue-600 text-white px-4 py-2 rounded-lg font-semibold shadow hover:from-cyan-600 hover:to-blue-700 transition-all duration-200"
          disabled={loading}
        >
          Add Slot
        </button>
                  ...
                  {editingId === slot.id ? (
                    <>
                      ...
                      <td className="p-2 flex gap-2">
                        <button onClick={() => handleUpdate(slot.id)} className="bg-green-500/80 hover:bg-green-600/90 text-white px-3 py-1 rounded">Save</button>
                        <button onClick={() => setEditingId(null)} className="bg-gray-500/80 hover:bg-gray-600/90 text-white px-3 py-1 rounded">Cancel</button>
                      </td>
                    </>
                  ) : (
                    <>
